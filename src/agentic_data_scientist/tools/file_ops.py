...
def _validate_path(path: str, working_dir: str) -> Path:
    ...
    ------
    ValueError
        If the path is outside the working directory or is a symlink escaping the sandbox
    """
    ...
    if Path(path).is_absolute():
        ...
        target_path = working_path / path

    # SECURITY: Check if path exists and is a symlink
    # If so, we need to verify the symlink target is within the working directory
    if target_path.is_symlink():
        # Get the symlink target
        try:
            symlink_target = target_path.readlink()
            # If relative, resolve against the parent directory
            if not symlink_target.is_absolute():
                symlink_target = (target_path.parent / symlink_target).resolve()
            # Verify the symlink target is within working directory
            try:
                symlink_target.relative_to(working_path)
            except ValueError:
                raise ValueError("Access denied: Symlink points outside the allowed working directory") from None
        except OSError:
            # If we can't read the symlink, deny access
            raise ValueError("Access denied: Cannot access symlink") from None

    # Resolve the path (follows symlinks that passed our check)
...
def directory_tree(
    ...
    try:
        ...
        def build_tree(current_path: Path) -> list[dict[str, str | list]]:
            ...
            try:
                # Get directory entries - wrap iterdir to handle PermissionError
                try:
                    dir_entries = list(current_path.iterdir())
                except PermissionError:
                    return []

                for entry in sorted(dir_entries, key=lambda x: x.name):
